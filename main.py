...
def main():
  ...
  # Obrazek do zadania 12
    ...
    if st.session_state.page == 'start':
        ...
        Prosimy, abyś przy ocenie wyborów kierował/a się wyłącznie przedstawionymi informacjami i analizą danych, a nie swoimi osobistymi preferencjami. Liczy się Twoja opinia o decyzji bazującej na obiektywnych kryteriach, które znajdziesz w opisie zadania.
        ...
        st.markdown(
            "<span style='color: red; font-weight: bold;'>Przechodząc do kolejnych stron badania, wyrażasz zgodę na udział i potwierdzasz, że zapoznałaś/eś się z powyższymi informacjami.</span>",
            unsafe_allow_html=True
        )

        if st.button("Rozpocznij") and not st.session_state.button_clicked:
            st.session_state.button_clicked = True
            go_to_next_page('question_1')

    elif st.session_state.page == 'question_1':
        st.title("Pytania Demograficzne")
        ...
        education_years = st.text_input("Ile lat edukacji ukończyłeś/aś do tej pory?", key="education_years")
        ...
        if st.session_state.group == 'control':
                st.subheader("Dla grupy wiedzącej, że odpowiedzi wybrało AI:")
                st.write("""
                W ramach tego badania otrzymasz serię 12 zadań. W każdym zadaniu przedstawimy Ci opisy trzech **różnych** pralek wraz z oceną, która z nich jest najlepszym wyborem. **Oceny-Wybory** i rekomendacje zostały wygenerowane przez system sztucznej inteligencji (AI).
                ...
                st.subheader("Dla grupy zakładającej, że odpowiedzi wybrała inna osoba badana:")
                st.write("""
                W ramach tego badania otrzymasz serię 12 zadań. W każdym zadaniu przedstawimy Ci opisy trzech **różnych** pralek wraz z oceną, która z nich jest najlepszym wyborem. **Oceny-Wybory** i rekomendacje zostały przygotowane przez inną osobę badaną.
        ...
        if st.button("Dalej") and not st.session_state.button_clicked:
            st.session_state.button_clicked = True
        ...
        if st.button("Dalej"):
            if any(value is None for value in [agree_choice, agree_scale, justification, clarity_scale, consideration_scale]):
                st.error("Upewnij się, że wszystkie pytania zostały wypełnione.")
            else:
                # Upewnij się, że 'responses' istnieje w session_state
                if 'responses' not in st.session_state:
                    st.session_state['responses'] = {}

                # Zapisz odpowiedzi dla danego zadania
        ...
        if st.button("Zakończ badanie i wyślij odpowiedzi", key="send_final"):
            if not all([final_q1, final_q2, final_q3, final_q4, final_q5]):
                st.error("Proszę odpowiedzieć na wszystkie pytania przed zakończeniem badania.")
                    ...
                    task_responses = "\n".join([
                        ...
                        for task_num, response in st.session_state.get('responses', {}).items()
                    ])

            user_responses = f"""
            Grupa: {st.session_state.group}
            Odpowiedzi demograficzne: {st.session_state.demographic_data}
            ...
            send_email(
                subject="Nowe odpowiedzi z badania",
                body=user_responses,
